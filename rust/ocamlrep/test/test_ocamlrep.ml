...
external get_byte_slice : unit -> Stdlib.Bytes.t = "get_byte_slice"
...
module SMap = Stdlib.Map.Make (struct
  type t = string
...
module SSet = Stdlib.Set.Make (struct
  type t = string
...
let test_int64 () =
  let cases = [Int64.max_int; Int64.min_int; 2L; 0xfaceb00cL] in
  List.iter (fun x -> assert (Int64.equal (roundtrip_int64 x) x)) cases
...
let test_byte_slice () =
  ...
  assert (String.equal (Stdlib.Bytes.sub_string b 0 4) "byte");
  assert (Char.equal (Stdlib.Bytes.get b 4) '\x00');
  assert (Char.equal (Stdlib.Bytes.get b 5) '\xFF');
  assert (String.equal (Stdlib.Bytes.sub_string b 6 5) "slice")
